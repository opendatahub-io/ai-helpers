...
as **anonymous (unnamed) fields** with `json:",inline"` tags. Use
`common.ComponentReleaseStatus \`json:",inline"\`` instead of a named field
like `Releases common.ComponentReleaseStatus` because a named field wraps
the inner `releases` array in an extra JSON object and fails CRD validation.
