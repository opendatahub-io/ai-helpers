...
**Pass:** Both embedded as anonymous (unnamed) fields with `json:",inline"`.
**Fail:** Missing either embed, using custom fields instead of the shared
types, or using a named field (e.g.,
`Releases common.ComponentReleaseStatus json:"releases,omitempty"`) which
serializes as a nested object instead of a flat array and breaks CRD
validation.
