...
def parse_task_block(block: str) -> Optional[dict]:
    ...
    header_match = re.match(
        r"^###?\s*(?:TASK-)?(\d+):?\s*(.+)$", lines[0], re.IGNORECASE
    )
    if not header_match:
        return None
    ...
    for line in lines[1:]:
        ...
        if subtask_match:
            ...
            task["subtasks"].append({"title": subtask_text, "completed": completed})
            continue
...
def create_requirements_json(task: dict) -> dict:
    ...
    return {
        ...
        "acceptance_criteria": [st["title"] for st in task["subtasks"]]
        or ["TODO: Define criteria"],
        "notes": task["notes"],
        "created_at": now,
        "updated_at": now,
...
def create_spec_directory(task: dict, output_dir: Path) -> Path:
    ...
    (spec_dir / "requirements.json").write_text(
        json.dumps(create_requirements_json(task), indent=2), encoding="utf-8"
    )

    # Write context.json
    (spec_dir / "context.json").write_text(
        json.dumps(create_context_json(), indent=2), encoding="utf-8"
    )

    # Write implementation_plan.json
    (spec_dir / "implementation_plan.json").write_text(
        json.dumps(create_implementation_plan(task), indent=2), encoding="utf-8"
    )
...
def main():
    ...
    parser.add_argument("--input", "-i", type=Path, help="Input TASKS.md file")
    parser.add_argument(
        "--output",
        "-o",
        type=Path,
