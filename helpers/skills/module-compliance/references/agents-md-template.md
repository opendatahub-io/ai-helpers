...
- The status struct must embed `common.Status` and
  `common.ComponentReleaseStatus` as **anonymous (unnamed) fields** with
  ...
  a named field wraps the inner `releases` array in an extra JSON object and
  fails CRD validation.
