---
name: rhaieng-commit-suggest
description: Generate RHAIENG Commits style commit messages or summarize existing commits
argument-hint: [N]
---

## Name
odh-ai-helpers:rhaieng-commit-suggest

## Synopsis
```bash
/rhaieng:commit-suggest       # Analyze staged changes
/rhaieng:commit-suggest [N]     # Analyze last N commits (1-100)
```

## Description
AI-powered command that analyzes code changes and generates commit messages following the project's RHAIENG format requirements.
...
**Use cases:**
- Create RHAIENG-formatted commit messages
- Improve or rewrite existing commits to meet project standards
- Generate squash messages for MR merges

**Difference from `/git:summary`** – That command is read-only, while `rhaieng:commit-suggest` generates actionable commit message suggestions for user review and manual use.
...
2. Analyze file paths and code content to determine appropriate RHAIENG ticket reference
3. Generate 3 RHAIENG-formatted commit message suggestions (Recommended, Standard, Minimal)
...
2. Parse commit messages and analyze changes to maintain RHAIENG format consistency
3. For **N=1**: Suggest improved rewrite following RHAIENG format
   For **N>=2**: Merge commits into unified RHAIENG-formatted squash message
4. Generate 3 RHAIENG-formatted commit message suggestions (Recommended, Standard, Minimal)
...
/rhaieng:commit-suggest

# Rewrite last commit message
/rhaieng:commit-suggest 1

# Summarize last 5 commits for squash
/rhaieng:commit-suggest 5
```

## Return Value

Generates 3 RHAIENG-formatted commit message suggestions:
...
- RHAIENG format title (`RHAIENG-XXX: description`)
- Blank line between title and body
- Body text explaining the changes in complete sentences
- Optional Jira integration (`Fixes RHAIENG-XXX`)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
RHAIENG-123: Add JWT authentication middleware
...
- **N=1** – Suggest improved rewrite for the last commit in RHAIENG format
- **N>=2** – Generate unified RHAIENG-formatted squash message with footer: `Squashed from N commits:` + original commit list
...
### Required Elements
<!-- skillsaw-disable content-placeholder-text -->
- **Title**: Must start with "RHAIENG-XXX:" followed by a short description
...
### Optional Elements
- **Jira Integration**: Include "Fixes RHAIENG-XXX" in the body to automatically close the Jira ticket when MR merges
...
- **RHAIENG Project Guidelines** – Internal project commit format requirements
- [Conventional Commits Specification](https://www.conventionalcommits.org/) – General industry standard (adapted for RHAIENG format)
