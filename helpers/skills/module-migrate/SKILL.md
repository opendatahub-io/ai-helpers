...
Search for webhook registrations related to the component:

```bash
grep -r "webhook" internal/controller/components/<name>/ || true
grep -r "webhook" internal/controller/services/<name>/ || true
grep -r "<ComponentKind>" internal/webhook/ || true
```
...
```bash
grep -rn "opendatahub-operator" internal/ api/ || true
grep -rn "openshift/api" internal/ api/ || true
```
